...
func IUpdateServicexxeScan(address string, client *http.Client, Red string, Green string, Yellow string, Reset string, Cyan string, currentTime string) {
	IUpdateServicexxe := "用友 NC IUpdateService XXE漏洞"
	...
	if strings.Contains(string(body22), "getDocument") {
		result18 := IUpdateServicexxe + "：" + urls
		...
		output := fmt.Sprintf("["+Green+"%s"+Reset+"] ["+Green+"+"+Reset+"] 存在%s：%s", currentTime, IUpdateServicexxe, urls)
		fmt.Println(output)
	} else {
		output := fmt.Sprintf("["+Cyan+"%s"+Reset+"] ["+Yellow+"-"+Reset+"] 不存在%s ", currentTime, IUpdateServicexxe)
		fmt.Println(output)
	}
}
