...
func UapwsauthScan(address string, client *http.Client, Red string, Green string, Yellow string, Reset string, Cyan string, currentTime string) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "An existing connection was forcibly closed by the remote host") || strings.Contains(err.Error(), "forcibly closed") {
			fmt.Printf("[%s%s%s] [%s-%s] %s 扫描时连接被目标服务器阻断\n", Cyan, currentTime, Reset, Yellow, Reset, uapwsauth)
		} else {
	...
	body7, err := ioutil.ReadAll(response.Body)
	if err != nil {
		output := fmt.Sprintf("["+Cyan+"%s"+Reset+"] ["+Red+"x"+Reset+"] %s 读取响应失败: %s", currentTime, uapwsauth, err)
		fmt.Println(output)
		return
	}
	if strings.Contains(string(body7), "showModule") {
		...
		output := fmt.Sprintf("[%s%s%s] [%s-%s] 不存在%s，状态码: %d", Cyan, currentTime, Reset, Yellow, Reset, uapwsauth, response.StatusCode)
		fmt.Println(output)
	}
}
