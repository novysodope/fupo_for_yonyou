...
func MeasureQResultActionScan(address string, client *http.Client, Red string, Green string, Yellow string, Reset string, Cyan string, currentTime string) {
	Portalreadfile := "用友 U8 Cloud MeasureQResultAction SQL注入漏洞"
	urls := address + "/service/~iufo/com.ufida.web.action.ActionServlet?action=nc.ui.iufo.query.measurequery.MeasureQResultAction&method=execute&selectQueryCondition=1"
	...
	if strings.Contains(string(body), "textFilter_keyDown") {
