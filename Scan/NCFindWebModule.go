...
func NCFindWebScan(address string, client *http.Client, Red string, Green string, Yellow string, Reset string, Cyan string, currentTime string) {
	...
	if response.StatusCode == http.StatusOK {
		body14, _ := ioutil.ReadAll(response.Body)
		if strings.Contains(string(body14), "jsp") {
			result6 := NCFindWeb + "：" + urls
			...
			output := fmt.Sprintf("["+Cyan+"%s"+Reset+"] ["+Yellow+"-"+Reset+"] 不存在%s ", currentTime, NCFindWeb)
			fmt.Println(output)
		}
	} else {
		output := fmt.Sprintf("["+Cyan+"%s"+Reset+"] ["+Yellow+"-"+Reset+"] 不存在%s ", currentTime, NCFindWeb)
