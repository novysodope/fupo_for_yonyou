...
func UapwsauhtScan(address string, client *http.Client, Red string, Green string, Yellow string, Reset string, Cyan string, currentTime string) {
	...
	if response.StatusCode == http.StatusNotModified {
		...
		body6, _ := ioutil.ReadAll(response.Body)
		if strings.Contains(string(body6), "ember2725") {
			result19 := uapwsauht + "：" + urls
		...
		output := fmt.Sprintf("["+Cyan+"%s"+Reset+"] ["+Yellow+"-"+Reset+"] 不存在%s ", currentTime, uapwsauht)
		fmt.Println(output)
	}
}
