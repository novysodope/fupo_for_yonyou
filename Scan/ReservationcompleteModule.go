...
func ReservationcompleteScan(address string, client *http.Client, Red string, Green string, Yellow string, Reset string, Cyan string, currentTime string) {
	Portalreadfile := "用友 U8CRM reservationcomplete SQL注入漏洞"
	urls := address + "/bgt/reservationcomplete.php?DontCheckLogin=1"
	...
	if response.StatusCode == http.StatusOK && response.ContentLength == 0 || len(body) == 0 {
		result := Portalreadfile + "：" + urls
		output := fmt.Sprintf("[%s%s%s] [%s+%s] 疑似存在%s：%s，请手动验证", Green, currentTime, Reset, Green, Reset, Portalreadfile, urls)
		fmt.Println(output)
