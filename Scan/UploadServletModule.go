...
func UploadServletScan(address string, client *http.Client, Red string, Green string, Yellow string, Reset string, Cyan string, currentTime string) {
	UploadServlet := "用友 NC UploadServlet反序列化漏洞"
	...
	// 读取响应内容
	body20, _ := ioutil.ReadAll(response.Body)

	if strings.Contains(string(body20), "EOFExcepti") {
		result18 := UploadServlet + "：" + urls
		...
		output := fmt.Sprintf("["+Green+"%s"+Reset+"] ["+Green+"+"+Reset+"] 存在%s：%s", currentTime, UploadServlet, urls)
		fmt.Println(output)
	} else {
		output := fmt.Sprintf("["+Cyan+"%s"+Reset+"] ["+Yellow+"-"+Reset+"] 不存在%s ", currentTime, UploadServlet)
		fmt.Println(output)
	}
}
